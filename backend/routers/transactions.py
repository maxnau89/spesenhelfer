...
async def upload_statement(report_id: str, file: UploadFile, db: AsyncSession = Depends(get_db)):
    ...
    # Also save permanently as statement.pdf for export later
    ...
    shutil.copy(tmp_path, statement_path)
    ...
    for p in parsed:
        tx = Transaction(
            ...
            amount=p.amount,
            currency=p.currency,
            needs_receipt=p.amount < 0,  # only debits need receipts by default
            raw_text=p.raw_text,
        )
        db.add(tx)
        created.append(tx)
...
async def list_transactions(report_id: str, db: AsyncSession = Depends(get_db)):
...
async def update_transaction(tx_id: str, body: TransactionUpdate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(Transaction).where(Transaction.id == tx_id).options(selectinload(Transaction.match))
    )
    ...
    if body.needs_receipt is not None:
        tx.needs_receipt = body.needs_receipt
    await db.commit()
    await db.refresh(tx)
    return tx
