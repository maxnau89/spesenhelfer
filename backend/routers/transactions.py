...
async def upload_statement(report_id: str, file: UploadFile, user: CurrentUser = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def list_transactions(report_id: str, user: CurrentUser = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def update_transaction(tx_id: str, body: TransactionUpdate, user: CurrentUser = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
